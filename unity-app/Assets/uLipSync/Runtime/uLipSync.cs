...
public class uLipSync : MonoBehaviour
{
    ...
    [Range(0.01f, 0.5f)] public float noDataTimeout = 0.1f;
    ...
    float _timeSinceDataReceived = 0f;
    ...
    void Update()
    {
        ...
        _timeSinceDataReceived += Time.deltaTime;

#if UNITY_WEBGL && !UNITY_EDITOR
        UpdateWebGL();
#endif

        if (_timeSinceDataReceived > noDataTimeout)
        {
            UpdateSilentResult();
            InvokeCallback();
            UpdateAudioSource();
    ...
    void UpdateResult()
    {
        ...
        if (sumScore > 0f)
        {
            ...
            mainPhoneme = profile.GetPhoneme(index);
        }
    ...
    void UpdateSilentResult()
    {
        ...
        if (profile)
        {
            for (int i = 0; i < profile.mfccs.Count; ++i)
            {
                ...
                if (_ratios.ContainsKey(phoneme)) continue;
                _ratios.Add(phoneme, 0f);
            }
        }
